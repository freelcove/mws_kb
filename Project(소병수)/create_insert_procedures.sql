...
is begin
insert into product_group_detail(prod_higher_group_num,prod_group_det_name) 
values(input_prod_higher,input_prod_group_det_name);
commit;
...
begin
if input_supplier is null then select corp_num into input_supplier2 
from corporation where input_own_corp=corp_num;
else
input_supplier2 := input_supplier;
end if;
insert into products(product_name, product_supplier, product_group, 
product_group_detail, product_in_price, product_out_price, product_stock_count,
product_own_corp)
values(input_name, input_supplier2, input_group, input_group_detail, 
input_in_price, input_out_price, input_stock, input_own_corp);
commit;
end insert_product;
...
begin
select sale_price into input_sale_price from sale_product 
where input_sale_code=sale_id;
select sale_corp_num into input_corp_num from sale_product 
where input_sale_code=sale_id;
insert into customer_orders(cust_id,sale_code,sale_price,order_amount,
delivery_address_country,delivery_address_city,delivery_address_street,corp_num)
values(input_cust_id,input_sale_code,input_sale_price,input_order_amount,
input_address_country,input_address_city,input_address_street,input_corp_num);
--고객 자산 감소
update users set user_balance=user_balance - total_price(input_sale_price,
input_order_amount) where input_cust_id=user_id;
--제품 재고수량 감소
update products set product_stock_count = product_stock_count - 
input_order_amount where products.product_code = (select sale_product_code 
from sale_product where input_sale_code = sale_id);
--판매자 자산 증가
update users set user_balance = user_balance + total_price(input_sale_price,
input_order_amount) 
where user_id = (select corp_owner_code from corporation 
where corp_num=input_corp_num);
commit;
end insert_order;
...
--주문자 기존 재고수량 복사
select product_stock_count into existing_amount from products 
where product_code = input_stocked_product;
--주문자 자산 감소
...
--주문자 제품 매입가 평균값으로 변경
update products set product_in_price = (total_price(product_in_price , existing_amount) + total_price(input_price , input_amount))/(input_amount + existing_amount)
where product_code = input_stocked_product;
--공급자 제품 재고수량 감소
